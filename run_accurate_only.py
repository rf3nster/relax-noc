############################################################################
# Script:
#    Runs RELAX testbench in accurate only mode, places results in CSVs and
#    .xlsx files
#
...
try:
    weights_file = open ('weights_baseline.dat','r')
except OSError:
...
print("Test type: Accurate-Only Random")
...
for weight_cnt in range(len(weights)):
    ...
    for injrate_cnt in range(len(injrates)):
        ...
        subprocess.call( ["vsim","-c",  "-do", "do ./test_scripts/noc_accurate_only.do"], cwd=os.getcwd(), stdout=subprocess.DEVNULL,  stderr=subprocess.DEVNULL)
        ############################################################################################################
    ...
    wb.save (filename = "./results/results_accurate_only.xlsx")  
